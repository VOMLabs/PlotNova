...
public class BlockEventListener implements Listener {
        ...
        TaskManager.runTaskLater(() -> {
            ...
            for (final PlotPlayer<?> player : PlotSquared.platform().playerManager().getPlayers()) {
                ...
                if (location.getWorldName().equals(world)) {
                    ...
                    ((PaperPlayer) player).player.sendBlockChange(bloc, data);
                }
            }
        }, TaskTime.ticks(3L));
    }
    ...
    public void onChange(BlockFromToEvent event) {
        ...
        final Location fromLocation = PaperUtil.adapt(fromBlock.getLocation());
        ...
        Location toLocation = PaperUtil.adapt(toBlock.getLocation());
        ...
        if (toPlot != null) {
            ...
            /*
               ...
               v
                <-----O-----> x
             */
            if (PaperUtil.adapt(location.clone().add(-1, 0, 1)  /* A */).getPlot() != null
                    || PaperUtil.adapt(location.clone().add(1, 0, 0)   /* B */).getPlot() != null
                    || PaperUtil.adapt(location.clone().add(1, 0, 1)   /* C */).getPlot() != null
                    || PaperUtil.adapt(location.clone().add(-1, 0, 0)  /* D */).getPlot() != null
                    || PaperUtil.adapt(location.clone().add(1, 0, 0)   /* E */).getPlot() != null
                    || PaperUtil.adapt(location.clone().add(-1, 0, -1) /* F */).getPlot() != null
                    || PaperUtil.adapt(location.clone().add(0, 0, -1)  /* G */).getPlot() != null
                    || PaperUtil.adapt(location.clone().add(1, 0, 1)   /* H */).getPlot() != null) {
                event.setCancelled(true);
            }
        }
    }
    ...
    public void onBlockPistonExtend(BlockPistonExtendEvent event) {
        ...
        if (area == null) {
            ...
            for (Block block1 : event.getBlocks()) {
                Location bloc = PaperUtil.adapt(block1.getLocation());
    ...
    public void onBlockDispense(BlockDispenseEvent event) {
        ...
        switch (type.toString()) {
            case "SHULKER_BOX", "WHITE_SHULKER_BOX", "ORANGE_SHULKER_BOX", "MAGENTA_SHULKER_BOX", "LIGHT_BLUE_SHULKER_BOX",
                ...
                Location location = PaperUtil.adapt(event.getBlock().getRelative(targetFace).getLocation());
                if (location.isPlotRoad()) {
                    event.setCancelled(true);
                    return;
                }
    ...
    public void onStructureGrow(StructureGrowEvent event) {
        ...
        Location location = PaperUtil.adapt(blocks.get(0).getLocation());
        PlotArea area = location.getPlotArea();
        if (area == null) {
            for (int i = blocks.size() - 1; i >= 0; i--) {
                location = PaperUtil.adapt(blocks.get(i).getLocation());
    ...
    public void onBigBoom(BlockExplodeEvent event) {
        ...
        if (area == null) {
            ...
            while (iterator.hasNext()) {
                location = PaperUtil.adapt(iterator.next().getLocation());
                if (location.isPlotArea()) {
                    iterator.remove();
                }
            }
        ...
        event.blockList().removeIf(blox -> !plot.equals(area.getOwnedPlot(PaperUtil.adapt(blox.getLocation()))));
    }
    ...
    public void onBlockIgnite(BlockIgniteEvent event) {
        ...
        Location location1 = PaperUtil.adapt(block.getLocation());
        ...
        if (player != null) {
            ...
            if (ignitingEntity != null) {
                ...
                if (igniteCause == BlockIgniteEvent.IgniteCause.FIREBALL) {
                    if (ignitingEntity instanceof Fireball) {
                        ...
                        if (fireball.getShooter() instanceof Entity shooter) {
                            location = PaperUtil.adapt(shooter.getLocation());
                ...
                Plot plotIgnited = PaperUtil.adapt(ignitingBlock.getLocation()).getPlot();
    ...
    public void onSpongeAbsorb(SpongeAbsorbEvent event) {
        ...
        Location location = PaperUtil.adapt(sponge.getLocation());
        PlotArea area = location.getPlotArea();
        List<org.bukkit.block.BlockState> blocks = event.getBlocks();
        if (area == null) {
            blocks.removeIf(block -> PaperUtil.adapt(block.getLocation()).isPlotArea());
        } else {
            Plot origin = area.getOwnedPlot(location);
            blocks.removeIf(block -> {
                Location blockLocation = PaperUtil.adapt(block.getLocation());
    ...
    public void onBlockMultiPlace(BlockMultiPlaceEvent event) {
        ...
        Location placedLocation = PaperUtil.adapt(event.getBlockReplacedState().getLocation());
        ...
        for (final BlockState state : event.getReplacedBlockStates()) {
            Location currentLocation = PaperUtil.adapt(state.getLocation());
            if (!pp.hasPermission(
                    Permission.PERMISSION_ADMIN_BUILD_ROAD
