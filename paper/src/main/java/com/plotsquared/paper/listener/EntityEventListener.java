...
public class EntityEventListener implements Listener {
    ...
    public EntityEventListener(
            final @NonNull PaperPlatform platform,
    ...
    private void onEntityDamageByEntityCommon(
        ...
        Location location = PaperUtil.adapt(damager.getLocation());
        if (!this.plotAreaManager.hasPlotArea(location.getWorldName())) {
            return;
        }
        if (!PaperEntityUtil.entityDamage(damager, victim, cause)) {
    ...
    public void creatureSpawnEvent(CreatureSpawnEvent event) {
        ...
        Location location = PaperUtil.adapt(entity.getLocation());
        ...
        switch (reason.toString()) {
            ...
            case "CUSTOM" -> {
                ...
                // Paper has getEntitySpawnReason() — no need for metadata
        ...
        if (PaperEntityUtil.checkEntity(entity, plot.getBasePlot(false))) {
            event.setCancelled(true);
        }
    }
    ...
    public void onBigBoom(EntityExplodeEvent event) {
        Location location = PaperUtil.adapt(event.getLocation());
        PlotArea area = location.getPlotArea();
        ...
        if (plot != null) {
            if (plot.getFlag(ExplosionFlag.class)) {
                ...
                while (iterator.hasNext()) {
                    ...
                    location = PaperUtil.adapt(block.getLocation());
    ...
    public void onPeskyMobsChangeTheWorldLikeWTFEvent(EntityChangeBlockEvent event) {
        ...
        if (e instanceof FallingBlock) {
            ...
            if (shooter instanceof Player) {
                PlotPlayer<?> pp = PaperUtil.adapt((Player) shooter);
                if (plot == null) {
            ...
            if (!(shooter instanceof Entity) && shooter != null) {
                ...
                Location sLoc =
                        PaperUtil.adapt(((BlockProjectileSource) shooter).getBlock().getLocation());
    ...
    public void onVehicleCreate(VehicleCreateEvent event) {
        ...
        Plot plot = area.getOwnedPlotAbs(location);
        if (plot == null || PaperEntityUtil.checkEntity(entity, plot)) {
            entity.remove();
            return;
        }
    ...
    public void onBeaconEffect(EntityPotionEffectEvent event) {
        if (event.getCause() != EntityPotionEffectEvent.Cause.BEACON) {
            return;
        }
        final Entity entity = event.getEntity();
        final Location location = PaperUtil.adapt(entity.getLocation());
        final Plot plot = location.getPlot();
        if (plot == null) {
            return;
        }
        final var effectsFlag = plot.getFlagContainer().getFlag(BeaconEffectsFlag.class);
        if (effectsFlag != null && !effectsFlag.getValue()) {
            event.setCancelled(true);
        }
    }
