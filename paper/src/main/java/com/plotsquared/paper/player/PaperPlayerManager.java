...
/**
 * Player manager providing {@link PaperPlayer Bukkit players}
 */
@Singleton
public class PaperPlayerManager extends PlayerManager<PaperPlayer, Player> {
    ...
    @NonNull
    @Override
    public PaperPlayer getPlayer(final @NonNull Player object) {
        if (object.getUniqueId().version() == 2) { // not a real player
            return new PaperPlayer(this.plotAreaManager, this.eventDispatcher, object, false, this.permissionHandler);
        }
        if (!object.isOnline()) {
    ...
    @Override
    public @NonNull PaperPlayer createPlayer(final @NonNull UUID uuid) {
        final Player player = Bukkit.getPlayer(uuid);
        if (player == null || !player.isOnline()) {
            throw new NoSuchPlayerException(uuid);
        }
        return new PaperPlayer(this.plotAreaManager, this.eventDispatcher, player, false, this.permissionHandler);
    }
