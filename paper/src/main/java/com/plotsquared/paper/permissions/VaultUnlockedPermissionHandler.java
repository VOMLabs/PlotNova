 ...
 */
package com.plotsquared.paper.permissions;
...
public class VaultUnlockedPermissionHandler implements PermissionHandler {
    ...
    public void initialize() {
        if (Bukkit.getServer().getPluginManager().getPlugin("VaultUnlocked") == null) {
            throw new IllegalStateException("VaultUnlocked is not present on the server");
        }
    ...
    public Optional<PermissionProfile> getPermissionProfile(
        ...
        if (playerPlotPlayer instanceof final PaperPlayer paperPlayer) {
            return Optional.of(new VaultUnlockedPermissionProfile(paperPlayer.getPlatformPlayer()));
    ...
    public Optional<PermissionProfile> getPermissionProfile(
        ...
        if (offlinePlotPlayer instanceof BukkitOfflinePlayer) {
            return Optional.of(new VaultUnlockedPermissionProfile(((BukkitOfflinePlayer) offlinePlotPlayer).player));
        }
        return Optional.empty();
    }
    ...
    private final class VaultUnlockedPermissionProfile implements PermissionProfile {

        private final OfflinePlayer offlinePlayer;

        private VaultUnlockedPermissionProfile(final @NonNull OfflinePlayer offlinePlayer) {
        ...
        public boolean hasPermission(
            ...
            if (world == null && offlinePlayer instanceof PaperPlayer) {
                return permissions.playerHas(((PaperPlayer) offlinePlayer).getPlatformPlayer(), permission);
            }
        ...
        public boolean hasKeyedPermission(
            ...
            if (world == null && offlinePlayer instanceof PaperPlayer) {
                return permissions.playerHas(
                        ((PaperPlayer) offlinePlayer).getPlatformPlayer(),
                        stub + ".*"
                ) || permissions.playerHas(((PaperPlayer) offlinePlayer).getPlatformPlayer(), stub + "." + key);
            }
