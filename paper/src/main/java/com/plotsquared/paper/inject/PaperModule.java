...
import com.plotsquared.paper.queue.PaperChunkCoordinator;
import com.plotsquared.paper.queue.PaperQueueCoordinator;
import com.plotsquared.paper.schematic.PaperSchematicHandler;
import com.plotsquared.paper.util.PaperChunkManager;
import com.plotsquared.paper.util.PaperInventoryUtil;
import com.plotsquared.paper.util.PaperRegionManager;
import com.plotsquared.paper.util.PaperSetupUtils;
import com.plotsquared.paper.util.PaperUtil;
import com.plotsquared.paper.util.fawe.FaweRegionManager;
import com.plotsquared.paper.util.fawe.FaweSchematicHandler;
...
public class PaperModule extends AbstractModule {
    ...
    private static final Logger LOGGER = LogManager.getLogger("PlotSquared/" + PaperModule.class.getSimpleName());

    private final PaperPlatform paperPlatform;

    public PaperModule(final @NonNull PaperPlatform paperPlatform) {
        this.paperPlatform = paperPlatform;
    }

    @Override
    protected void configure() {
        bind(PlayerManager.class).to(PaperPlayerManager.class);
        bind(JavaPlugin.class).toInstance(paperPlatform);
        bind(PlotPlatform.class).toInstance(paperPlatform);
        bind(PaperPlatform.class).toInstance(paperPlatform);
        ...
        bind(InventoryUtil.class).to(PaperInventoryUtil.class);
        bind(SetupUtils.class).to(PaperSetupUtils.class);
        bind(WorldUtil.class).to(PaperUtil.class);
        install(new FactoryModuleBuilder()
                .implement(ProgressSubscriber.class, DefaultProgressSubscriber.class)
                .build(ProgressSubscriberFactory.class));
        bind(ChunkManager.class).to(PaperChunkManager.class);
        if (PlotSquared.platform().isFaweHooking()) {
            ...
            bind(SchematicHandler.class).to(PaperSchematicHandler.class);
            bind(RegionManager.class).to(PaperRegionManager.class);
        }
        bind(GlobalBlockQueue.class).toInstance(new GlobalBlockQueue(QueueProvider.of(PaperQueueCoordinator.class)));
        ...
        install(new FactoryModuleBuilder()
                .implement(ChunkCoordinator.class, PaperChunkCoordinator.class)
