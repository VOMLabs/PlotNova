...
@Singleton
public class PaperUtil extends WorldUtil {

    public static final LegacyComponentSerializer LEGACY_COMPONENT_SERIALIZER = LegacyComponentSerializer.legacySection();
    public static final LegacyComponentSerializer LEGACY_COMPONENT_SERIALIZER_AMPERSAND = LegacyComponentSerializer.legacyAmpersand();
    public static final MiniMessage MINI_MESSAGE = MiniMessage.builder().build();
    private static final Logger LOGGER = LogManager.getLogger("PlotSquared/" + PaperUtil.class.getSimpleName());
     ...
     */
    public static @NonNull PaperPlayer adapt(final @NonNull Player player) {
        final PlayerManager<?, ?> playerManager = PlotSquared.platform().playerManager();
        return ((PaperPlayerManager) playerManager).getPlayer(player);
    }
    ...
    public static @NonNull Location adapt(final org.bukkit.@NonNull Location location) {
        return Location
                .at(
                        com.plotsquared.paper.util.PlatformWorld.of(location.getWorld()),
    ...
    private static <T> void ensureMainThread(final @NonNull Consumer<T> consumer, final @NonNull T value) {
        if (Bukkit.isPrimaryThread()) {
            ...
            TaskManager.runTask(() -> consumer.accept(value));
        }
    }
    ...
    public void getHighestBlock(final @NonNull String world, final int x, final int z, final @NonNull IntConsumer result) {
        ensureLoaded(world, x, z, chunk -> {
            final World paperWorld = Objects.requireNonNull(getWorld(world));
            // Skip top and bottom block
            int air = 1;
            int maxY = com.plotsquared.paper.util.PlatformWorld.getMaxWorldHeight(paperWorld);
            int minY = com.plotsquared.paper.util.PlatformWorld.getMinWorldHeight(paperWorld);
            for (int y = maxY - 1; y >= minY; y--) {
                Block block = paperWorld.getBlockAt(x, y, z);
            ...
            result.accept(paperWorld.getMaxHeight() - 1);
        });
    }
    ...
    public int getHighestBlockSynchronous(final @NonNull String world, final int x, final int z) {
        ...
        return paperWorld.getMaxHeight() - 1;
    }
    ...
    public Set<BlockVector2> getChunkChunks(com.plotsquared.core.location.World<?> world) {
        ...
        World paperWorld = ((com.plotsquared.paper.util.PlatformWorld) world).getPlatformWorld();
        if (Bukkit.isPrimaryThread()) {
            for (Chunk chunk : paperWorld.getLoadedChunks()) {
