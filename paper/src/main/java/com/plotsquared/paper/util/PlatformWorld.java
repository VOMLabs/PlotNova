...
public class PlatformWorld implements World<org.bukkit.World> {

    // Upon world unload we should probably have the P2 PlatformWorld be GCed relatively eagerly, thus freeing the bukkit world.
    //  We also want to avoid circumstances where a bukkit world has been GCed, but a P2 PlatformWorld has not
    private static final Map<String, WeakReference<PlatformWorld>> worldMap = Maps.newHashMap();
    ...
    private PlatformWorld(final org.bukkit.World world) {
        this.world = new SoftReference<>(world);
    }

    /**
     * Get a new {@link PlatformWorld} from a world name
     *
     * @param worldName World name
     * @return World instance
     */
    public static @NonNull PlatformWorld of(final @NonNull String worldName) {
        final org.bukkit.World platformWorld = Bukkit.getWorld(worldName);
        if (platformWorld == null) {
            throw new IllegalArgumentException(String.format("There is no world with the name '%s'", worldName));
        }
        return of(platformWorld);
    }

    /**
     * Get a new {@link PlatformWorld} from a Bukkit world
     *
     * @param world Bukkit world
     * @return World instance
     */
    public static @NonNull PlatformWorld of(final org.bukkit.World world) {
        WeakReference<PlatformWorld> platformWorldRef = worldMap.get(world.getName());
        PlatformWorld platformWorld;
        if (platformWorldRef != null && (platformWorld = platformWorldRef.get()) != null && world.equals(platformWorld.world.get())) {
            return platformWorld;
        }
        platformWorld = new PlatformWorld(world);
        worldMap.put(world.getName(), new WeakReference<>(platformWorld));
        return platformWorld;
    }
    ...
    public boolean equals(final Object o) {
        ...
        final PlatformWorld that = (PlatformWorld) o;
    ...
    public String toString() {
        return "PlatformWorld(world=" + this.world + ")";
    }
