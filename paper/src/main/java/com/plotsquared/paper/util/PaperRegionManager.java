...
@Singleton
public class PaperRegionManager extends RegionManager {

    private final GlobalBlockQueue blockQueue;

    @Inject
    public PaperRegionManager(
    ...
    public int[] countEntities(@NonNull Plot plot) {
        ...
        World world = PaperUtil.getWorld(area.getWorldName());
        ...
        if (doWhole) {
                ...
                PaperSupport.getChunkAtAsync(location).thenAccept(chunk -> {
                    if (chunks.contains(chunk)) {
                        ...
                        if (X > bx && X < tx && Z > bz && Z < tz) {
                            ...
                            Plot other = area.getPlot(PaperUtil.adapt(location));
            ...
            for (Chunk chunk : chunks) {
                ...
                for (Entity entity : entities1) {
                    if (X == bx || X == tx || Z == bz || Z == tz) {
                        Plot other = area.getPlot(PaperUtil.adapt(entity.getLocation()));
                        if (plot.equals(other)) {
                            count(count, entity);
                        }
                    } else {
    ...
    public void clearAllEntities(@NonNull Location pos1, @NonNull Location pos2) {
        ...
        final World paperWorld = PaperUtil.getWorld(world);
        final List<Entity> entities;
        if (paperWorld != null) {
            entities = new ArrayList<>(paperWorld.getEntities());
        } else {
