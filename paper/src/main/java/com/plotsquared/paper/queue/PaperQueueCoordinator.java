 ...
 */
package com.plotsquared.paper.queue;

import com.google.inject.Inject;
import com.plotsquared.paper.schematic.StateWrapper;
import com.plotsquared.paper.util.PaperBlockUtil;
...
public class PaperQueueCoordinator extends BasicQueueCoordinator {
    ...
    private org.bukkit.World paperWorld;
    @Inject
    ...
    @Inject
    public PaperQueueCoordinator(@NonNull World world) {
        super(world);
    }

    @Override
    public BlockState getBlock(int x, int y, int z) {
        Block block = getPaperWorld().getBlockAt(x, y, z);
        return PaperBlockUtil.get(block);
    }
    ...
    private void setWorldBlock(int x, int y, int z, @NonNull BaseBlock block, @NonNull BlockVector2 blockVector2, boolean edge) {
        try {
            ...
            if (getChunkObject() instanceof Chunk chunkObject) {
                ...
                existing = getPaperWorld().getBlockAt(x, y, z);
            }
            final BlockState existingBaseBlock = BukkitAdapter.adapt(existing.getBlockData());
            if (PaperBlockUtil.get(existing).equals(existingBaseBlock) && existing.getBlockData().matches(blockData)) {
                return;
            }
    ...
    private org.bukkit.World getPaperWorld() {
        if (paperWorld == null) {
            paperWorld = Bukkit.getWorld(getWorld().getName());
        }
        return paperWorld;
    }
